	...
	public function check_userlogin () {
		...
		if ( $this->phpass->CheckPassword( $user_password, $fetch['user_password'] ) ) {
			...
			if ( session_id() != $fetch['user_session_id'] && ! $post ) { 
				$this->logged_in = false;
				$this->login_error = 'ID de sessão inválido.';
			...
			// Verifica se existe foto no perfil do usuário, se nao existir, exibe a padrão
			if ( empty( $_SESSION['userdata']['user_photo'] ) ){
				$_SESSION['userdata']['user_photo'] = 'user.png';
			}			
